...
for dataset in "${DATASETS_ARR[@]}"; do
  echo "[PDCC-MER] Diagnostics retraining dataset=$dataset"
  ...
  if [[ -f "$PROJECT_DIR/pdcc_multiseed_tools/analyze_retrained_rccr.py" ]]; then
    "$PYTHON" pdcc_multiseed_tools/analyze_retrained_rccr.py \
      --dataset "$dataset" \
      --data-path "$DATA_PATH" \
      --run-root "$RUN_ROOT" \
      --seeds "$SEEDS_CSV" \
  ...
  if [[ -f "$PROJECT_DIR/pdcc_multiseed_tools/analyze_retrained_tplr.py" ]]; then
    "$PYTHON" pdcc_multiseed_tools/analyze_retrained_tplr.py \
      --dataset "$dataset" \
      --data-path "$DATA_PATH" \
      --run-root "$RUN_ROOT" \
      --seeds "$SEEDS_CSV"
...
if [[ -f "$PROJECT_DIR/pdcc_multiseed_tools/summarize_retrain_diagnostics.py" ]]; then
  "$PYTHON" pdcc_multiseed_tools/summarize_retrain_diagnostics.py --root "$RUN_ROOT"
fi

echo "[PDCC-MER] Diagnostic runs completed."
